...
import { getPartyCardDeck } from '@/data/party-cards'
import { Card, Game, Player, Round } from '@/types/types'
...
const useGame = (game: Game, player?: Player | null) => {
  ...
  const discardCard = (card: Card): Game | null => {
    ...
    const playerIndex = getPlayerIndexById(game, player.id)

    if (player && playerIndex !== null && playerIndex !== undefined) {
      const newGame = { ...game }
  ...
  const drawCardFromDiscardDeck = () => {
    ...
    // remove top card from deck
    newGame.discardDeck = newGame.discardDeck.slice(
      0,
      newGame.discardDeck.length - 1
    )
